...
const Message = ({ message, currentChat }) => {
  ...
  const getFileUrl = (file) => {
    if (!file || !file.url) {
      return file?.path ? `http://localhost:3000/${file.path}` : ""
    }

    // If the URL already has the complete URL, return it as is
    if (file.url.startsWith("http")) {
      return file.url
    }

    // Otherwise, add the base URL
    return `http://localhost:3000/${file.url.replace(/^\//, "")}`
  }
  ...
  return (
    <div className={`flex ${message.sender === "me" ? "justify-end" : "justify-start"}`}>
      {message.sender !== "me" && (
        <img
          src={getProfilePictureUrl(currentChat.profilePicture) || "/placeholder.svg"}
      ...
      >
        ...
        {message.text && (
          <div className="px-4 py-2">
            <p>{message.text}</p>
            <span className={`text-xs block mt-1 ${message.sender === "me" ? "text-red-100" : "text-gray-500"}`}>
              {message.timestamp}
            </span>
          </div>
        )}
