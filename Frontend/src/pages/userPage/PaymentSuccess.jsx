  ...
  useEffect(() => {
    const createSchedule = async () => {
      ...
      try {
        ...
        // Check if we have a pidx from Khalti in the URL
        const pidx = searchParams.get("pidx")

        // If we have a pidx, verify the payment first
        if (pidx) {
          const verifyResponse = await axios.get(
            `http://localhost:3000/api/payment/khalti-success?pidx=${pidx}&bookingId=${bookingDetails.bookingNumber}&amount=${bookingDetails.price}`,
          )
          ...
          // Payment is already verified and booking updated by the backend
        ...
        // If no pidx, create schedule with paid status
        const response = await axios.post("http://localhost:3000/api/availability/createSchedule", {
          ...
          paymentStatus: "paid",
          isClientVerified: true, // Explicitly set client verification to true
        })
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="max-w-md w-full mx-auto p-8 bg-white rounded-lg shadow-lg">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-6">Your booking has been confirmed and your trainer has been notified.</p>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <button
              onClick={() => navigate("/trainerExplore")}
